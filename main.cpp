...
// 向KVStore插入随机键值对
template<typename T>
void insertRandomKeys(KVStore<T>& store, std::mutex& storeMutex, int threadId, int numInsertsPerThread) {
    // 随机数生成器
...
// 测试KVStore的插入和读取功能
template<typename T>
void testKVStoreInsert(const std::string& dataFilename, const std::string& metaFilename, int numThreads, int numInsertsPerThread) {
    KVStore<T> store(dataFilename, metaFilename, 16); // 128MB buffer
    std::mutex storeMutex; // 用于保护store的互斥量

    // 创建线程
    std::vector<std::thread> threads;
    for (int i = 0; i < numThreads; ++i) {
        threads.emplace_back(insertRandomKeys<T>, std::ref(store), std::ref(storeMutex), i, numInsertsPerThread);
    }
...
int main() {
    ...
    testKVStoreInsert<int>(dataFilename, metaFilename, numThreads, numInsertsPerThread);
